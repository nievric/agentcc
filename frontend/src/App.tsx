...
import { CheckoutSelector, BranchTasks } from "./Worktrees";
import { CheckoutRequest, waitForOperation, AgentActivity, api, ConversationMessage, DisplaySettings, RegisteredModel, Session, SessionOutput, Summary, Telemetry, Workspace, WorkspaceStorageSettings, WorkspaceUsage } from "./api";
...
export default function App() {
  ...
  const [launchWorktreeId, setLaunchWorktreeId] = useState<string | null>(null);
  ...
  const openLaunch = (workspaceId?: string, worktreeId?: string) => {
    setLaunchWorktreeId(worktreeId ?? null);
  ...
  const deleteWorkspace = async (workspaceId: string, mode: "soft" | "hard", discardHistory = false) => {
    await api.deleteWorkspace(workspaceId, mode, discardHistory);
    await refresh();
  ...
  return (
    <main className="app-shell">
      ...
      {showLaunch && <LaunchDialog workspaces={data.workspaces} models={data.models} initialWorkspaceId={launchWorkspaceId} initialWorktreeId={launchWorktreeId} onClose={() => { setShowLaunch(false); setLaunchWorkspaceId(null); }} onCreated={onCreated} />}
...
function Content({ view, data, selected, terminalSession, logSession, onSelect, onSelectTerminal, onSelectLog, onOpenTerminal, onOpenLogs, onOpenIde, onLaunch, onCreateWorkspace, onDeleteWorkspace, onRestoreWorkspace, onRefresh }: { view: View; data: AppData; selected?: Session; terminalSession?: Session; logSession?: Session; onSelect: (id: string) => void; onSelectTerminal: (id: string) => void; onSelectLog: (id: string) => void; onOpenTerminal: (id: string) => void; onOpenLogs: (id: string) => void; onOpenIde: (id: string) => void; onLaunch: (workspaceId?: string, worktreeId?: string) => void; onCreateWorkspace: () => void; onDeleteWorkspace: (id: string, mode: "soft" | "hard", discardHistory?: boolean) => Promise<void>; onRestoreWorkspace: (id: string) => Promise<void>; onRefresh: () => Promise<void> }) {
...
function FleetView({ data, selected, onSelect, onOpenTerminal, onOpenLogs, onOpenIde, onLaunch }: { data: AppData; selected?: Session; onSelect: (id: string) => void; onOpenTerminal: (id: string) => void; onOpenLogs: (id: string) => void; onOpenIde: (id: string) => void; onLaunch: (workspaceId?: string, worktreeId?: string) => void }) {
  const cards = [
...
function WorkspaceView({ workspaces, deletedWorkspaces, sessions, onLaunch, onCreateWorkspace, onOpenIde, onDelete, onRestore }: { workspaces: Workspace[]; deletedWorkspaces: Workspace[]; sessions: Session[]; onLaunch: (workspaceId?: string, worktreeId?: string) => void; onCreateWorkspace: () => void; onOpenIde: (sessionId: string) => void; onDelete: (id: string, mode: "soft" | "hard", discardHistory?: boolean) => Promise<void>; onRestore: (id: string) => Promise<void> }) {
  ...
  const remove = async (workspace: Workspace, mode: "soft" | "hard", discardHistory = false) => {
    try { setMessage(null); await onDelete(workspace.id, mode, discardHistory); setPendingDelete(null); }
...
function WorkspaceDeleteDialog({ workspace, onClose, onDelete }: { workspace: Workspace; onClose: () => void; onDelete: (workspace: Workspace, mode: "soft" | "hard", discardHistory?: boolean) => Promise<void> }) {
  ...
  const choose = async (mode: "soft" | "hard") => {
    ...
    try { await onDelete(workspace, mode, mode === "hard" && discardHistory); }
...
function LaunchDialog({ workspaces, models, initialWorkspaceId, initialWorktreeId, onClose, onCreated }: { workspaces: Workspace[]; models: RegisteredModel[]; initialWorkspaceId: string | null; initialWorktreeId: string | null; onClose: () => void; onCreated: () => Promise<void> }) {
  const [checkout, setCheckout] = useState<CheckoutRequest | null>(null);
  const launchKey = useRef({ payload: "", key: "" });
  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
